...
from keras.layers import Dense, Dropout, TimeDistributed, Bidirectional, Input, Embedding, Softmax
...
#     embed = Embedding(input_dim=1, output_dim=8, embeddings_initializer="RandomUniform", input_length=7)
    

    encoder = RecurrentSequential(unroll=unroll, stateful=stateful,
                                  return_sequences=True)
    
    encoder.add(LSTMCell(hidden_dim, kernel_initializer="RandomUniform", recurrent_initializer="RandomUniform", 
                         batch_input_shape=(shape[0], shape[2])))
...
#     decoded = Softmax(axis=-1)(decoded)
